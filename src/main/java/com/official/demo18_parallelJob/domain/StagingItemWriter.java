...
public class StagingItemWriter<T> extends JdbcDaoSupport implements StepExecutionListener, ItemWriter<T> {
	...
	public void write(final List<? extends T> items) {
		...
		BatchPreparedStatementSetter preparedStatement = new BatchPreparedStatementSetter() {
			@Override
		...
		String sql = "INSERT into BATCH_STAGING (ID, JOB_ID, VALUE, PROCESSED) values (?,?,?,?)";

		getJdbcTemplate().batchUpdate( sql, preparedStatement);
	}
